...
//今後の修正内容（あとでドキュメントに直す）

//エンカウント方法の変更(なんかキャタピーしか出てこん)
//ボールで捕まえることに関する処理（Item = 1 なら捕まえるコマンドが実行可能）
//HP回復手段が何もないから何とかする（きずぐすりor回復ポイント？？）
//マップループについて→できれば周りを木で囲いたい 

//requestAnimationFrame  実行環境（フレームレートとかによってキャラの動くスピードが変わってしまったりするけど）